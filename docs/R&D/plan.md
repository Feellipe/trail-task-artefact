...
- Use `npx create-next-app@latest task-artefact --typescript --app --src-dir --eslint --tailwind --no-import-alias`.
...
- `@types/react@^19.x`
- `eslint@^9.x`
- `tailwindcss@^4.x` (if not already installed by create-next-app)

**Why:** Section 13.2 lists these as required development dependencies.
...
- Path aliases are configured if desired (e.g., `@/*` mapping to `./src/*`).

**Why:** TypeScript configuration must align with Next.js 15 requirements and support clean imports across the project.
...
### Step 1.6 -- Create `.gitignore` and `README.md`

**What:** Ensure `.gitignore` includes `node_modules/`, `.next/`, `out/`, and other standard Next.js ignores. Create a `README.md` with usage instructions.

**Why:** Section 14 lists these files. Standard project hygiene.
...
**What:** Create Zod schemas in `src/types/task.ts` alongside the type. The schemas are:
...
**Why:** Section 5.3 specifies exact Zod schemas with precise constraints and error messages. These schemas are used by both tRPC procedures (server-side validation) and the client-side form hook (`useTaskForm`).
...
- Import schemas from `src/types/task.ts`.
...
- Import `appRouter` from `_app.ts` and `createTRPCContext` from `context.ts`.
...
- `createCallerFactory` from `@trpc/server`.
- A `createServerCaller` function that creates a caller using `appRouter` and `createTRPCContext`.
- This caller is used by Server Components to invoke tRPC procedures directly on the server without HTTP.

**Why:** Section 2.3 describes the server-side caller. Section 6.2 describes it as essential for SSR data fetching while respecting the tRPC context and middleware chain.

**Key technical considerations:**
- Import `createCallerFactory` from `@trpc/server`.
- The caller factory is called once (or the caller is created once per request in Server Components).
...
- A `TRPCProvider` component (or `Providers` component) that:
  ...
  - Includes the `sonner` `<Toaster />` component (Section 12.7 -- placed inside the provider wrapper, SSR-safe without additional mounting logic).
...
- The `sonner` `<Toaster />` goes inside this provider.
...
**What:** Create `src/components/task/TaskCard.tsx` as a Server Component. Displays:
...
- A `DeleteTaskButton` component (Client Component, so it is composed inside the card).
- Accepts `task: Task` as a prop.

**Why:** Section 12.2 specifies TaskCard as a Server Component. Section 9.3 mandates all user input is rendered as text content via JSX interpolation.

**Key technical considerations:**
- Since TaskCard is a Server Component but contains `DeleteTaskButton` (a Client Component), TaskCard simply renders `<DeleteTaskButton taskId={task.id} />` as a child -- this is valid because Server Components can render Client Components as long as no non-serializable props are passed.
- The `Task` type crossing the boundary must only use serializable types (Section 12.3), which it does.
...
- Optional: optimistic update (Section 6.2 mentions this as optional).
...
- [ ] `TaskCard` renders task data correctly in a server-rendered context.
...
- Creates a server-side tRPC caller via `createServerCaller()` (from `caller.ts`).
- Creates a `QueryClient` (server-side instance, not the stable client one).
- Calls `prefetchQuery` on `queryClient` for the first page of tasks: `caller.task.list({ cursor: undefined, limit: 10 })`.
...
**Why:** Section 12.1 specifies the listing page as a Server Component with prefetchQuery + HydrationBoundary. Section 3.1 describes the complete SSR flow. Section 6.2 describes the Hydration-Resistant State pattern.

**Key technical considerations:**
- Import `HydrationBoundary`, `QueryClient`, `dehydrate` from `@tanstack/react-query`.
- Import `createServerCaller` from `src/server/trpc/caller.ts`.
- The server-side `QueryClient` is created fresh per request (not via useRef -- that is only for the client provider).
- `prefetchQuery` uses `queryClient.prefetchQuery({ queryKey: ['task.list'], queryFn: () => caller.task.list({ cursor: undefined, limit: 10 }) })`.
...
**Why:** Section 12.1 specifies the edit page as a Server Component that awaits async params and fetches task data via the server-side caller. Section 12.3 mandates only serializable props across the RSC boundary. Section 12.8 specifies the Next.js 15 async params pattern.

**Key technical considerations:**
- The `task` object is already JSON-serializable (all fields are strings or null) -- no special serialization needed.
- Do NOT wrap in `HydrationBoundary` -- the form is a Client Component that receives props directly; it does not use React Query for initial data (it receives it as props).
...
1. **Zod schema file location:** The architecture doc shows Zod schema code snippets in Section 5.3 without specifying an exact file. Two reasonable options exist: (a) co-locate schemas in `src/types/task.ts` alongside the `Task` interface, or (b) create a dedicated `src/server/trpc/schemas.ts`. Option (a) keeps types and validation together; option (b) keeps server-side validation separate from shared types. Which is preferred? Note: if schemas must be imported by both the client-side hook (`useTaskForm`) and the server-side router, they must live in a shared location (not exclusively under `src/server/`).

2. **Path alias convention:** The architecture doc folder structure (Section 14) does not specify whether `@/` path aliases are used. The `create-next-app` scaffolding typically sets up `@/*` mapping to `./src/*`. Should the project use `@/` imports or relative paths? This affects every import statement across the entire project.

3. **Tailwind CSS:** Section 13.2 lists `tailwindcss@^4.x` as a development dependency but marks it as "(optional)". Is Tailwind CSS actually desired for styling, or should plain CSS / CSS Modules be used? If Tailwind is not used, the `tailwindcss` dependency and any `tailwind.config.ts` should be removed.

4. **TaskCard composition with Client Components:** `TaskCard` is specified as a Server Component (Section 12.2), but it contains `DeleteTaskButton` (a Client Component) and `TaskCardDate` (also a Client Component). Additionally, the "Edit" link inside TaskCard likely needs to use `next/link`, which is a Server Component-compatible import. However, if `TaskCard` is rendered inside `TaskList` (which is a Client Component), then `TaskCard` effectively runs on the client anyway. Is it still architecturally meaningful to mark `TaskCard` as a Server Component, or should it be made a Client Component for simplicity?

5. **Edit page HydrationBoundary:** The architecture doc does not explicitly mention using `HydrationBoundary` on the edit page. The edit page is a Server Component that fetches task data via the caller and passes it as a prop to `TaskForm` (Client Component). However, `TaskForm` uses tRPC mutations which need the tRPC client context. The `TRPCProvider` in the root layout provides this. Is there any need for `HydrationBoundary` on the edit page, or is direct prop passing sufficient?

6. **Optimistic updates for delete:** Section 6.2 mentions optimistic updates as "optional" for delete mutations. Should this be implemented in the initial build, or deferred? If implemented, the `onMutate` callback needs to cancel outgoing queries, snapshot the previous state, remove the task from the cache optimistically, and roll back on `onError`.

7. **Cursor format validation:** Section 5.3 defines `cursor` as `z.string().optional()`, but Section 11.4 specifies it should be a base64-encoded ISO timestamp. The Zod schema does not validate that the cursor is valid base64 or a valid ISO timestamp after decoding. Should additional validation be added (e.g., a custom Zod refinement that decodes and validates), or is the loose `z.string().optional()` acceptable for a prototype?

8. **Listing page initial data in infinite query:** When `app/page.tsx` (Server Component) prefetches the first page and passes it via `HydrationBoundary`, the `TaskList` Client Component uses `trpc.task.list.useInfiniteQuery()`. The query key used in `prefetchQuery` on the server must match the query key that `useInfiniteQuery` generates on the client. In tRPC v11, the query key is auto-generated as `[['task', 'list'], { input: { cursor: undefined, limit: 10 } }]`. The server-side `prefetchQuery` must use this exact key structure. Is there a documented tRPC v11 pattern for matching these keys, or should we use `trpc.task.list.getQueryKey()` on the server-side caller/utils?

9. **`createServerCaller` export pattern:** The architecture doc (Step 3.6) describes the server-side caller but does not specify its exact export name or whether it exports a factory function or a ready-to-use caller instance. Should `caller.ts` export (a) a `createServerCaller` function that returns a new caller each time it is called, or (b) a pre-created singleton caller? Option (a) is more aligned with per-request patterns; option (b) is simpler for a prototype.

10. **sonner toast configuration:** The `toast` function from `sonner` is called imperatively inside mutation callbacks (`onSuccess`, `onError`). The `<Toaster />` component is rendered in `TRPCProvider.tsx`. Is there a need to configure the `Toaster` component with specific options (e.g., `position`, `richColors`, `closeButton`), or are the defaults acceptable?
